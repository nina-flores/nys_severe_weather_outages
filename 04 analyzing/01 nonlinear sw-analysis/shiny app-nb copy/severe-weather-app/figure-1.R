...
setwd("~/Desktop/projects/casey cohort/power outages/severe weather PO NYS/code/extreme weather/05 shiny app/severe-weather-app/data")
data <- read.fst("data.fst")

data_u <- data %>% filter(urbanicity  == "u")
data = data_u %>% filter(severe_weather_variable == "abs_wind_speed_knots") %>%
  mutate(RR = fit.rr)
...
fig <- fig %>% layout(
  scene = list(
    xaxis = list(title = "Windspeed (kts)"),
