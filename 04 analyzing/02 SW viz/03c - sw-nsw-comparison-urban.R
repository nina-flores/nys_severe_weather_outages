...
outages_u <- read.fst("outages_u_duration.fst") 
outages_u_defined <- read.fst("sw_outages_u_defined.fst")

outages_u <-full_join(outages_u,outages_u_defined) %>%
  filter(i_gt_90pct == 1)
...
data_u = full_join(shape, outages_u)

data_u_customers_out = data_u %>%
  select(ind_90pct, dps_id, prop_out, start_outage, urn)

data_u_duration = data_u %>%
...
data_u_filt <- data_u %>% filter(start_outage == 1)
setDT(data_u_filt)
data_u_filt = data_u_filt[, sw := ifelse(rowSums(.SD, na.rm = TRUE) >= 1, "Severe weather driven", "Non-severe weather driven"), 
...
data_u_filt <- data_u_filt  %>% select(ind_90pct, sw, urn)

### calculate the average percentage without power during outages by severe weather driven and not:

data_pct_co = full_join(data_u_customers_out, data_u_filt) %>%
...
data_duration = full_join(data_u_duration, data_u_filt) %>%
  group_by(sw, dps_id, urn) %>%
  summarize(avg_dur = mean(duration)) 

### calculate the frequency by severe weather driven and not:
data_freq = full_join(data_u_n, data_u_filt) %>%
  group_by(sw, dps_id, urn) %>%
  summarize(freq = n())
...
pdf("urban_comparison_maps.pdf", width = 8, height = 11.5)
p
dev.off()
