...
for(i in run){
  ...
  df <- df %>%
    ...
    select(duration, dps_id, datetime_eastern, ind_90pct, svi) %>%
    ungroup()
...
# for next steps, to make maps with average duration and customers out for all
#  outages, will also use this to make 1+ and 8+ maps overall 
setwd("~/Desktop/projects/casey cohort/power outages/severe weather PO NYS/data/outage-final/sw outage indicators")
write.fst(outages_u_dur, "outages_w_duration_u.fst")
write.fst(outages_r_dur, "outages_w_duration_r.fst")
write.fst(outages_n_dur, "outages_w_duration_n.fst")


# for next steps, to make maps with average duration and customers out for 
# sw outages, will also use this to make 1+ and 8+ maps by each sw type
