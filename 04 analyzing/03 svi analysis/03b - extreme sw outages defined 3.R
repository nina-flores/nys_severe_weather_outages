...
for(i in run){
  ...
  assign(paste0("sw_outages_",i,"_3"), get(paste0("outages_",i,"_defined"))%>%
           group_by(dps_id) %>%
           mutate(sum_cold_outages = sum(cold_driven_outage, na.rm = T),
                  sum_cold_outages_bin = ifelse(sum_cold_outages >= 3, 1,0),
                  sum_heat_outages = sum(heat_driven_outage, na.rm = T),
                  sum_heat_outages_bin = ifelse(sum_heat_outages >= 3, 1,0),
                  sum_wind_outages = sum(wind_driven_outage, na.rm = T),
                  sum_wind_outages_bin = ifelse(sum_wind_outages >= 3, 1,0),
                  ...
                  sum_snow_outages_bin = ifelse(sum_snow_outages >= 3, 1,0),
                  sum_ltn_outages = sum(lightning_driven_outage, na.rm = T),
                  sum_ltn_outages_bin = ifelse(sum_ltn_outages >= 3, 1,0)) %>%
           select(dps_id, 
                  svi,
...
setwd("~/Desktop/projects/casey cohort/power outages/severe weather PO NYS/data/outage-final/sw outage indicators")
write.fst(sw_outages_u_3, "sw_outages_u_3.fst")
write.fst(sw_outages_r_3, "sw_outages_r_3.fst")
write.fst(sw_outages_n_3, "sw_outages_n_3.fst")
