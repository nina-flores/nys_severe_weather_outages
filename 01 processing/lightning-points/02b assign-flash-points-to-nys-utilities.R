...
daily_lightning_exposure_complete <- as.data.frame(lighting_utility_daily) %>% 
  ...
  complete(time_utc_daily = as.character(c(
    seq.POSIXt(
      ...
      by = "day")))) %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(num_daily_exposure = ifelse(is.na(num_daily_exposure) == TRUE & time_utc_daily >= 2017-03-01, 0, num_daily_exposure)) %>%
  dplyr::mutate(bin_exposure_daily = ifelse(is.na(bin_exposure_daily) == TRUE & time_utc_daily >= 2017-03-01 , 0, bin_exposure_daily))
...
daily_lightning_exposure_complete<- full_join(utility_day, daily_lightning_exposure_complete)%>% 
  dplyr::mutate(num_daily_lightning_exposure = ifelse(is.na(num_daily_exposure) == TRUE & time_utc_daily >= 2017-03-01, 0, num_daily_exposure)) %>% # and its not before march 2017
  dplyr::mutate(bin_exposure_lightning_daily = ifelse(is.na(bin_exposure_daily) == TRUE& time_utc_daily >= 2017-03-01, 0, bin_exposure_daily)) # and its not before march 2017
